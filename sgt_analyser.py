...
parser.add_argument('-m', '--method', help = "Choose a method for the tangent-fitting", choices=['cut_off', 'linear_area', 'µ_max'], required = True)
...
if arg.method == 'linear_area' and (arg.upper_boundary is None or arg.lower_boundary is None):
    parser.error('using --method "linear area" requires --upper_boundary and --lower_boundary.')

#define arguments as variables:
board_count = int(arg.checkerboard_nr)
first_wells = arg.first_well
last_wells = arg.last_well
control_wells =arg.control_well
...
if tangent_fitting_method == 'cut_off':
    ...
    #if TRUE ignore the specified numpy warning:
...
def curve_fit_calc(dataframe, actual_well):
    ...
    #initialize data:
    x_data = np.array(actual_dataframe_part['Time'].values, dtype=float)
...
#defines function to compute intersection-points between two graphs over the
#same x-axis:
def intersection_point(first_values_list, second_values_list, actual_element):
    
    #calculate theindex of x- and y-values where the intersection point occurs:
    intersection_point = np.argwhere(np.diff(np.sign(first_values_list
                                     - second_values_list[actual_element]))).flatten()
    
    return intersection_point


#defines function which calculates µ at a intersection-point between two graphs:
def µ_calc_for_intersection_point(x_values_list, y_values_list, 
                                  intersection_point_list, actual_element):
    
    #get the index of the x- & y-value at the intersection-point:
    intersection_point_index = intersection_point_list[actual_element]
    
    #calculate delta-x & -y at the intersection point:
    delta_x = np.subtract(x_values_list[intersection_point_index],
                          x_values_list[intersection_point_index - 1])
    delta_y = np.subtract(y_values_list[actual_element][intersection_point_index],
                          y_values_list[actual_element][intersection_point_index - 1])
...
def µ_calc_by_boundaries(dataframe, actual_well, upper_boundary, lower_boundary):
    ...
    #searches the nearest fitted values to the user-specified boundaries for the
    #actual well:
...
def log_calc(dataframe, actual_row, first_well, last_well, control_well,
    ...
    #get sgt of actual well:
    actual_well_dataframe_part = dataframe[dataframe['Well'] == actual_well_name]
    sgt_value_of_actual_well = actual_well_dataframe_part['SGT'].values[0]

    #check that sgt-value of actual well is no string:
    if type(sgt_value_of_actual_well) != str:
        
        #if TRUE set variable to TRUE:
        well_sgt_available = True

    #check if control-well is a string -> one control-well given:
    if type(control_well) == str:

        #get sgt of control-well:
        control_well_dataframe_part = dataframe[dataframe['Well'] == control_well]
        sgt_of_control_well = control_well_dataframe_part['SGT'].values[0]

        #check that sgt-value of actual well is no string:
        if type(sgt_of_control_well) != str:
            
            #if TRUE set variable to TRUE:
            control_sgt_available = True
    
    #otherwise check if control-well is a list -> multiple control-wells given:
    elif type(control_well) == list:
        
        #get list of sgt-values of all control-wells that are no string:
        sgt_of_control_well_list = list(dataframe[dataframe['Well'] == element]['SGT'].values[0]
                                   for element in control_well if
                                   type(dataframe[dataframe['Well'] == element]['SGT'].values[0]) != str)
        
        #check if any sgt-value is in the list or if all were strings:
        if len(sgt_of_control_well_list) != 0:
    ...
    if  actual_well_letter >= first_well[0] and actual_well_letter <= last_well[0]:
        if actual_well_number >= int(first_well[1:]) and actual_well_number <= int(last_well[1:]):
            ...
            if well_sgt_available == True and control_sgt_available == True:
                ...
                SGT_subtracted_value = (int(sgt_value_of_actual_well) 
                                        - int(sgt_of_control_well))
...
#checkerboard:
def BBC_determination_per_board(dataframe, checkerboard_list,
                                first_antibiotic_column, second_antibiotic_column,
                                actual_board):
    ...
    first_antibiotic_concentrations, second_antibiotic_concentrations = map(list, zip(*list(BBC_well_determination(dataframe,
                                                                                                                   first_well,
                                                                                                                   last_well,
    ...
    #list:
    first_antibiotic_concentrations = [inner for outer in (element for element in first_antibiotic_concentrations 
...
def fici_values_per_board_calc(dataframe, checkerboard_list, actual_board, 
                               first_antibiotic_first_bbc_value_list, 
                               second_antibiotic_first_bbc_value_list, 
...
def first_in_row(well_list, actual_letter, first_well_in_row_list,
                 well_min_letter_and_number):
    ...
    if len(first_well_in_row_list) > 0:
        ...
        if first_well_in_row_list[-1][1:] == end_well_number:   
            ...
            well_selection = [well_name for well_name in well_list
                              if actual_letter in well_name]
...
#with an log-level-reduction >= 3:
def first_in_column(well_list, actual_number, first_well_in_column_list,
                    well_min_letter_and_number):
...
def first_well_identifier(checkerboard_list, actual_board, fici_values_list):
    ...
    #/numbers:
    first_well_in_row = list(first_in_row(wells, letters, first_well_in_row, 
                                          well_min_letter_and_number)
                             for letters in well_letters)
    first_well_in_column = list(first_in_column(wells, numbers, first_well_in_column,
                                                well_min_letter_and_number)
                                for numbers in well_numbers)

    #deletes 'None'-elements & duplicates in the lists:
    first_well_in_row = list(element for element in first_well_in_row 
                             if element != None)
    first_well_in_row = list(dict.fromkeys(inner for outer in first_well_in_row 
                                           for inner in outer))
    
    first_well_in_column = list(element for element in first_well_in_column 
                                if element != None)
    first_well_in_column = list(dict.fromkeys(inner for outer in first_well_in_column 
                                              for inner in outer))
...
def FICI_finder(fici_values_list, first_wells_in_rows_and_columns_list,
                actual_board, actual_element):
...
def append_FICI_values_to_board(fici_values_with_well_list, actual_board):
    ...
    board_fici_values = list(element[1] for element 
                             in fici_values_with_well_list[actual_board])
...
def tangent_value_calc(dataframe_one, dataframe_two, actual_well, time_values):
    ...
    if type(µ) != str:  
        ...
        calc_tangent_values = [[actual_well, time,((time * µ) + n)] 
...
for i in tqdm(range(1), desc = 'Calculate µ'):
    ...
    #check if the user want to perform the tangent fitting automated by just
    #setting the OD cut-off:
    if tangent_fitting_method == 'cut_off':
        
        #get list of all time values/wells & delete duplicates:
        time_values_list = np.unique(data_merged['Time'].values)
        well_plate = np.unique(data_merged['Well'].values)

        #create an array of the same length as 'time_values_list' and fill all
        #positions with the cut-off-value:
        cut_off_values_list = np.repeat(cut_off, len(time_values_list))

        #get an list containing arrays, where each holds all fitted values for
        #an specific well
        ...
        wells_with_intersection = np.delete(well_plate, empty_intersection_list_index)
        fitted_values_list_with_intersection = np.delete(fitted_values_list, 
                                                         empty_intersection_list_index, 
                                                         axis = 0)
        
        #get the fitted-/time-value at the intersection-point for each well:
        fitted_values_at_intersection = list(fitted_values_list_with_intersection[element][intersection_point_index_list_with_intersection[element]] 
                                             for element in range(len(intersection_point_index_list_with_intersection)))
        time_values_at_intersection = list(time_values_list[intersection_point_index_list_with_intersection[element]] 
                                           for element in range(len(intersection_point_index_list_with_intersection)))


        #calls function 'µ_calc' for every element wells_with_intersection:
        µ_values_at_intersection = list(µ_calc_for_intersection_point(time_values_list, 
                                        fitted_values_list_with_intersection, 
        ...
        values_array_for_wells_with_intersection = list((wells_with_intersection[element], 
                                                         time_values_at_intersection[element], 
                                                         fitted_values_at_intersection[element], 
                                                         µ_values_at_intersection[element]) 
                                                         for element in range(len(wells_with_intersection)))
        
        #calls function 'n_calc' for every element in 'values_array_for_wells
        #_with_intersection' & splits output into two lists:
        y0_list, unneeded_list = map(list, zip(*list(n_calc(elements) for elements 
                                    in values_array_for_wells_with_intersection)))

        #creates dataframes from the resulting arrays:
        data = pd.DataFrame(y0_list, columns = ['Well', 'µ_max', 'y0'])

        #summarise well-name, time- & µ-value at intersection-point of each well:
        µ_values_list = list((values_array_for_wells_with_intersection[element][0],
                              values_array_for_wells_with_intersection[element][1][0],
                              values_array_for_wells_with_intersection[element][3][0])
                             for element in range(len(values_array_for_wells_with_intersection)))
        
        #create a dataframe from the resulting array:
        data_µ_max = pd.DataFrame(µ_values_list, columns = ['Well', 'Time', 'µ_max'])
    ...
    #
    #
    #check if the user want to perform the tangent-fitting by setting a linear
    ...
    #
    #
    #check if the user want to perform the tangent-fitting automated via the
    #µ_max of each growth-curve:
    elif tangent_fitting_method == 'µ_max':
...
## synergism between the two antibiotics.
    ...
    fici_values_with_well_per_board = list(FICI_finder_per_board(boards,
                                                                 fici_values,
...
for i in tqdm(range(1), desc = 'Creating diagramms'):
    ...
    #check if tangent-fitting-method 'cut-off' is used:
    if tangent_fitting_method == 'cut_off':
        
        #if TRUE overwrite 'well_plate' to avoid plotting conflicts:
        well_plate = wells_with_intersection
    ...
    if tangent_fitting_method == 'cut_off':
        ...
        domain = ['raw data', 'fitted data', 'tangent', 'cut-off point']
